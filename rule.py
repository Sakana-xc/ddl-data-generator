...
def build_rule_dict(row: pd.Series):
    ...
    item_name = str(row.get(COL_ITEM_NAME, "")).strip()
...
def df_to_validation_code(sheet_name: str, df: pd.DataFrame):
    ...
    code = []
    code.append("validation_rules = {")
    code.append(f'    "{py_escape(sheet_name)}": {{')
    code.append('        "config": [')
    code.extend(config_lines if config_lines else ['        '])
    code.append("        ],")
    code.append('        "validation_rule": {')
    code.extend(rule_lines if rule_lines else ['        '])
    code.append("        }")
    code.append("    }")
    code.append("}")

    return "\n".join(code), work_df, pd.DataFrame(todo_list)
...
st.write("Excelから validation_rules 定義を生成します。")
...
if uploaded_file is not None:
    try:
        ...
        if st.button("コード生成", type="primary"):
            ...
            st.dataframe(work_df[[COL_ITEM_NAME, COL_ATTR, COL_LENGTH, COL_SCREEN, COL_REQUIRED]], use_container_width=True)
